...
public class TechGlobalStaticTablesTest extends TechGlobalBase {
    ...
    public void validateStaticTables() {
        ...
        for (int i = 0; i < headers.length; i++) {
            ...
            // IntStream.range(0, headers.length).forEach(i ->Assert.assertEquals(techGlobalStaticTablesPage.tableHeader.get(i).getText(), headers[i]));
        }
    }
    ...
    @Test(priority = 2, description = "Validate first row of table")
    ...
    @Test(priority = 1, description = "Validate 4th Column of the table")
